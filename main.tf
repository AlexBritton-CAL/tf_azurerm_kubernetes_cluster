locals {
  ...
  kubernetes_cluster_name = coalesce(try(local.kube_defaults.generate_name, null), try(var.config.generate_name, null), var.generate_name) ? "${var.resource_prefix}-${var.instance_name}-aks" : var.name

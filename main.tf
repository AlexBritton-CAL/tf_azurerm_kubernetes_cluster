...
resource "azurerm_kubernetes_cluster" "this" {
  ...
  default_node_pool {
    name                         = coalesce(local.kube_default.default_node_pool.name, var.default_node_pool.name)
    min_count                    = coalesce(local.kube_default.default_node_pool.min_count, var.default_node_pool.min_count)
    node_count                   = coalesce(local.kube_default.default_node_pool.node_count, var.default_node_pool.node_count)
    max_count                    = coalesce(local.kube_default.default_node_pool.max_count, var.default_node_pool.max_count)
    auto_scaling_enabled         = coalesce(local.kube_default.default_node_pool.auto_scaling_enabled, var.default_node_pool.auto_scaling_enabled)
    vm_size                      = coalesce(local.kube_default.default_node_pool.vm_size, var.default_node_pool.vm_size)
    temporary_name_for_rotation  = coalesce(local.kube_default.default_node_pool.temporary_name_for_rotation, var.default_node_pool.temporary_name_for_rotation)
    only_critical_addons_enabled = coalesce(local.kube_default.default_node_pool.only_critical_addons_enabled, var.default_node_pool.only_critical_addons_enabled)
    zones                        = coalesce(local.kube_default.default_node_pool.zones, var.default_node_pool.zones)
    vnet_subnet_id               = coalesce(local.kube_default.default_node_pool.vnet_subnet_id, var.default_node_pool.vnet_subnet_id)
    node_public_ip_enabled       = coalesce(local.kube_default.default_node_pool.node_public_ip_enabled, var.default_node_pool.node_public_ip_enabled)
  ...
  maintenance_window_auto_upgrade {
    frequency   = coalesce(local.kube_default.maintenance_window_auto_upgrade.frequency, var.maintenance_window_auto_upgrade.frequency)
    interval    = coalesce(local.kube_default.maintenance_window_auto_upgrade.interval, var.maintenance_window_auto_upgrade.interval)
    duration    = coalesce(local.kube_default.maintenance_window_auto_upgrade.duration, var.maintenance_window_auto_upgrade.duration)
    day_of_week = coalesce(local.kube_default.maintenance_window_auto_upgrade.day_of_week, var.maintenance_window_auto_upgrade.day_of_week)
    start_time  = coalesce(local.kube_default.maintenance_window_auto_upgrade.start_time, var.maintenance_window_auto_upgrade.start_time)
    utc_offset  = coalesce(local.kube_default.maintenance_window_auto_upgrade.utc_offset, var.maintenance_window_auto_upgrade.utc_offset)
  }

  maintenance_window_node_os {
    frequency   = coalesce(local.kube_default.maintenance_window_node_os.frequency, var.maintenance_window_node_os.frequency)
    interval    = coalesce(local.kube_default.maintenance_window_node_os.interval, var.maintenance_window_node_os.interval)
    duration    = coalesce(local.kube_default.maintenance_window_node_os.duration, var.maintenance_window_node_os.duration)
    day_of_week = coalesce(local.kube_default.maintenance_window_node_os.day_of_week, var.maintenance_window_node_os.day_of_week)
    start_time  = coalesce(local.kube_default.maintenance_window_node_os.start_time, var.maintenance_window_node_os.start_time)
    utc_offset  = coalesce(local.kube_default.maintenance_window_node_os.utc_offset, var.maintenance_window_node_os.utc_offset)
  }
  workload_autoscaler_profile {
    keda_enabled                    = local.kube_default.workload_autoscaler_profile.keda_enabled
    vertical_pod_autoscaler_enabled = local.kube_default.workload_autoscaler_profile.vertical_pod_autoscaler_enabled
  }
...
module "azurerm_kubernetes_cluster_node_pool" {
  for_each                      = local.kube_default.node_pools
